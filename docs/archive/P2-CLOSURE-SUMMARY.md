# P2 Closure Summary (Archive)

**Canonical status:** [P2-ACCEPTANCE.md](../P2-release-gate/P2-ACCEPTANCE.md)

Дата: 2026-03-07 (обновлено после синхронизации API/contracts)
...
- Operations: `POST /api/v1/devices/:serial/rotate-mqtt` для ротации MQTT credentials устройства (admin), plaintext пароль возвращается один раз.
...
- `docs/archive/MASTER-SPEC.md` — P1 source of truth.
...
- Статус/критерии: `docs/P2-release-gate/P2-ACCEPTANCE.md`
- `docs/api-reference.md` — unified API reference (P1 + P2 + compatibility).
- `docs/archive/openapi-p1.json` — P1 baseline.
- `docs/P2/openapi-p2.json` — P2 additions/changes.
- `docs/openapi.json` — combined актуальный контракт.
- `docs/P2-hardening/HARDENING-SPEC.md` — architecture hardening decisions (CLOSED).
- `docs/P2-hardening/HARDENING-SUMMARY.md` — итоги hardening-цикла.
...
## 9) Architecture Hardening (post-closure)

Выполнен отдельный hardening-цикл по `docs/P2-hardening/HARDENING-SPEC.md`:

- Onboarding переработан как guided 5-шаговый flow (claim/assign/issue/activate/verify) — `/onboard`.
- Устранён лишний reconcile Mosquitto в `provision.ts`.
- `README.md` актуализирован: deployment profiles, JWT auth, роли, webhook v2.
- Все acceptance criteria HARDENING-SPEC §10 выполнены.

Подробнее: `docs/P2-hardening/HARDENING-SUMMARY.md`.

## 10) Security Hardening (post-closure)
...
- Audit integrity:
  - actor derives from JWT auth context; `?actor=` does not override audit actor.
