...
Machine-readable схема callback payload: `components/schemas/AlertCallbackPayload` в `docs/openapi-p1.json`.
...
- API_TOKEN requests: actor = `"api_token"` (or deprecated `?actor=...`)
- `?actor=` больше не применяется к JWT flow
